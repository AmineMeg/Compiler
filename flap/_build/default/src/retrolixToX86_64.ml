...
module InstructionSelector : InstructionSelector =
  struct
    ...
    let mov ~(dst : dst) ~(src : src) =
      insns begin
      match (src, dst) with
      | `Addr addr_s, `Addr addr_d -> [movq ~src ~dst:r15; movq ~src:r15 ~dst]
    ...
    let div ~dst ~srcl ~srcr =
      insns (movq srcl rax :: cqto :: movq srcr r15 :: idivq r15 :: movq rax dst :: [])
...
module FrameManager(IS : InstructionSelector) : FrameManager =
  struct
    ...
    let frame_descriptor ~params ~locals =
      (* retourne la map, avec l'ajout de l *)
       let rec add_locals map pos locs = 
          match locs with
          | [] -> map
          | h::t -> let map = S.IdMap.add h (Mint.of_int (pos)) map in 
          add_locals map (pos - Mint.size_in_bytes) t
        in
          let map = S.IdMap.empty 
        in
          let map = add_locals map (-8) locals
        in
      { 
